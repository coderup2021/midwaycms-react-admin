...
export function getIdPath(id: number, cateTree: Cate[]): string {
  const paths: number[] = []
  let flag = false
  let end = false
  function _handle(id: number, cateTree: Cate[]) {
    for (let i = 0; i < cateTree.length; i++) {
      if (flag) return
      const cate = cateTree[i]
      if (cate.id === id) {
...
export function addRootCate(cateTree: Cate[]): Cate[] {
  const root: Cate = {
    id: 0,
    name: '根目录',
    parentId: 0,
